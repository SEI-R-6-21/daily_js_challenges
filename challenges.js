...
function isPalindrome(string) {
  ...
  if (
    string.length === 0 ||
    string.length === 1 ||
    ans.toLowerCase() === string.toLowerCase()
  ) {
    return true
  } else {
    return false
...
function primeFactors(n) {
  let ans = []
  if (n < 2 || !Number.isInteger(n)) return ans
  let prime = 2
  while (!isPrime(n)) {
...
function balancedBrackets(string) {
  ...
  for (let i = 0; i < string.length; i++) {
    ...
    if ('([{'.includes(c)) {
      ...
      if (!'() {} []'.includes(stack.pop() + c)) return false
    }
  }
  return true
}
...
function getNumForIP(bits) {
  let arr = bits.split('.').reverse()
  let sum = 0
  for (let i = 0; i < arr.length; i++) {
    sum += parseInt(arr[i]) * 256 ** i
  }
...
function gridTrip(arr, string) {
  ...
  while (i < string.length) {
    ...
    ans[lookup[dir][0]] += numString * lookup[dir][1]
  }
  return ans
}
/*-----------------------------------------------------------------
