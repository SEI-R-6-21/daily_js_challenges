...
function addList() {
  ...
  for (i = 0; i < arguments.length; i++) {
    listNums += arguments[i]
  }
  return listNums
}
addList(5, 8) //an argument is in the parenthesis of a function, if it is not an array.
/*-----------------------------------------------------------------
...
//   }
//   return backwardString.toUpperCase()
// }
// reverseUpcaseString()
/*-----------------------------------------------------------------
...
function isPalindrome(string) {
  ...
  const reverseString = stringArrayRev.join('')
  let lowerCaseRev = reverseString.toLowerCase()
  let lowerCaseString = string.toLowerCase()
  if (lowerCaseString === lowerCaseRev) {
    return true
  } else {
    return false
  }
...
function mumble(string) {
  let str = ''
  for (let i = 0; i < string.length; i++) {
    str += ((i || '') && '-') + string.charAt(i).repeat(i + 1)
  }
  return str
}
/*-----------------------------------------------------------------
...
function findHighestPriced(arr) {
  ...
  for (let i = 0; i < arr.length; i++) {
    arr[i].price > arr[m].price ? (m = i) : (m = m)
  }
  return arr[m]
}
/*-----------------------------------------------------------------
...
function totalTaskTime(arr, n) {
  ...
  while (arr.length > n) {
    ...
    time += shortestthreads = threads.map((t) => t - shortest).filter((t) => t)
