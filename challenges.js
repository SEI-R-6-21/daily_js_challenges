...
function mumble(string) {
  let res = ''
  for (let i = 0; i < string.length; i++) {
    res = res + ((i || '') && '-') + string.charAt(i).repeat(i + 1)
  }
  return res
}
/*-----------------------------------------------------------------
...
function fromPairs(arr) {
  ...
  const arrToObj = (nestedArr) => {
    newObj[nestedArr[0]] = nestedArr[1]
  }

  arr.forEach(arrToObj)
  return newObj
}
/*-----------------------------------------------------------------
...
function mergeObjects(obj1, ...additionalObj) {
  return Object.assign(obj1, ...additionalObj)
}
/*-----------------------------------------------------------------
...
function findHighestPriced(arr) {
  return arr.reduce((max, item) => (item.price > max.price ? item : max))
}
/*-----------------------------------------------------------------
...
function flatten(arr) {
  ...
  arr.forEach((x) => {
    if (Array.isArray(x)) {
      newArray = newArray.concat(flatten(x))
    } else {
      newArray.push(x)
    }
  })
...
function intersection(arr1, arr2) {
  ...
  arr1.forEach((i) => {
    let x = arr2Copies.indexOf(i)
    if (x > -1) {
      res.push(arr2Copies.splice(x, 1)[0])
    }
  })
  return res
}
...
function balancedBrackets(string) {
  ...
  return string.split('').every((i) => {
    if ('([{'.includes(i)) {
      return arr.push(i)
    } else {
      return '() {} []'.includes(arr.pop() + i)
    }
  })
}
/*-----------------------------------------------------------------
...
function getNumForIP(bits) {
  return bits.split('.').reduce((int, val) => int * 256 + +val)
}
/*-----------------------------------------------------------------
...
function gridTrip(arr, string) {
  ...
  string.forEach(function (i) {
    let trip = i.charAt(0)
    arrXY[search[trip][0]] += i.substr(1) * search[trip][1]
  })
  return arrXY
}
/*-----------------------------------------------------------------
