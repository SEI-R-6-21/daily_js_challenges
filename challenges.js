...
function isPalindrome(string) {
  ...
  let string2 = string.replaceAll(' ', '').toLowerCase()
...
function mumble(string) {
  ...
  for (let i = 0; i < string.length; i++) {
    for (let j = 0; j <= i; j++) {
      solution = solution + string[i]
    }
    if (string.length > 1 && i != string.length - 1) {
      solution = solution + '-'
    }
  }
  return solution
...
function primeFactors(n) {
  let nums = []
  if (n <= 1 || !Number.isInteger(n)) return nums
...
function intersection(arr1, arr2) {
  ...
  let _arr2 = [...arr2]
  arr1.forEach((val) => {
    let index = _arr2.indexOf(val)
    if (index > -1) ans.push(_arr2.splice(index, 1)[0])
  })
  return ans
}
/*-----------------------------------------------------------------
...
function balancedBrackets(string) {
  ...
  for (let i = 0; i < string.length; i++) {
    ...
    if ('([{'.includes(b)) {
      ...
      if (!'(){}[]'.includes(arr.pop() + b)) return false
    }
  }
  return true
}
