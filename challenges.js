...
const reverseUpcaseString = (string) => {
  ...
  let splitString = upperCased.split('')
...
const removeEnds = (string) => {
  ...
  if (string.length < 3 ) {
    ...
    const withoutFirstAndLast = string.slice(1, -1) 
...
function isPalindrome(string) {
  ...
  let newString = string.replace(/\W+|_/g, "").toLowerCase()
...
function intersection(arr1, arr2) {
  ...
  arr1.forEach((val) => {
    ...
    if (idx > -1) result.push(arr2Copy.splice(idx, 1)[0])
  })
  return result
}
/*-----------------------------------------------------------------
...
function getNumForIP(bits) {
  return bits.split(".").reduce((int, value) => int * 256 + +value)
}
/*-----------------------------------------------------------------
