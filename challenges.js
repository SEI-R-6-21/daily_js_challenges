...
function computeRemainder(n1, n2) {
  if(n2 == 0) return Number.POSITIVE_INFINITY
...
function range(n1, n2) {
  ...
  array = [...Array(n2 - n1).keys()].map(i => i + n1)
  return array
}
/*-----------------------------------------------------------------
...
function reverseUpcaseString(string) {
  ...
  strArray.map((word, i)=>{
    i == strArray.length - 1 ? str += word : str += `${word} `
  })
...
function charCount(string) {
  ...
  for(let i=0; i < string.length; i++){
    ...
    Object.keys(stringObj).includes(letter) ? stringObj[letter] +=1 : stringObj[letter] = 1
  }
...
function isPalindrome(string) {
  ...
  return reversedStr.toLowerCase() == string.toLowerCase() ? true : false
}
/*-----------------------------------------------------------------
...
function reduceArray(arr, cb, value) {
  ...
  arr.map((elem, i) =>{
    initialAcc = cb(initialAcc, elem, i)
  })
  return initialAcc
}
/*-----------------------------------------------------------------
...
function intersection(arr1, arr2) {
  ...
  let array2 = [...arr2]
  arr1.map((value) => {
    let i = array2.indexOf(value)
    if (i > -1) result.push(array2.splice(i, 1)[0])
  })
  return result
}
/*-----------------------------------------------------------------
...
function getNumForIP(bits) {
  let splitBits = bits.split('.').reverse()
  var sum = 0
  splitBits.map((bit, i)=> {
    sum += parseInt(bit) * 256 ** i
  })
  return sum
}
/*-----------------------------------------------------------------
