...
function reverseUpcaseString(string) {
  ...
  for ( let i = string.length-1; i >= 0; i-- ) {
    reversedStr += string[i].toUpperCase()
  }
  return reversedStr
}
/*-----------------------------------------------------------------
...
function removeEnds(string) {
  if ( string.length >= 3 ) {
    let newStr = string.substr(1,string.length-2)
    return newStr
  }
  else {
    return ''
...
function hammingDistance(str1, str2) {
  ...
  if ( str1.length != str2.length ) { return NaN } 
    ...
    for ( let i = 0; i < str1.length; i++ ) {
      str1[i] != str2[i] ? difs++ : console.log( 'same' )
    }
    return difs 
  }
...
function mumble(string) {
  ...
  let mumble = ''
  for ( let i = 0; i < string.length; i++ ) {
    mumble += ( string[i].repeat( i+1 ))
    mumble += '-' 
  }
  if ( mumble[mumble.length-1] === '-' ) { mumble = mumble.slice( 0,-1 ) }
  return mumble
...
function mergeObjects(obj1, obj2, obj3) {
  ...
  for ( let i = 0; i < Object.keys(obj2).length; i++ ) {
    obj1[Object.keys(obj2)[i]] = obj2[Object.keys(obj2)[i]] 
  }
  if ( obj3 ) {
    for ( let i = 0; i < Object.keys(obj3).length; i++ ) {
      obj1[Object.keys(obj3)[i]] = obj3[Object.keys(obj3)[i]] 
    }
  } 
  return obj1
...
function primeFactors(n) {
  ...
  const isPrime=(n)=>{
    ...
    for (let i=2;i<=n/2;i++) {
      if (Number.isInteger(n/i)) return false}return true
  }
