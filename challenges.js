...
function mumble(string) {
  let expectation = "";
  for (let i = 0; i < string.length; i++) {
    expectation += ((i || "") && "-") + string[i].repeat(i + 1);
  }
  return expectation;
}
/*-----------------------------------------------------------------
...
function intersection(arr1, arr2) {
  let filteredArray = arr1.filter(function (n) {
...
function balancedBrackets(string) {
  ...
  let stack = [];
  for (let i = 0; i < string.length; i++) {
    let breaker = string.charAt(i);
    if ("([{".includes(breaker)) {
      stack.push(breaker);
    } else {
      if (!"() {} []".includes(stack.pop() + breaker)) return false;
    }
  }
  return true;
}
...
function getNumForIP(bits) {
  let splitter = bits.split(".").reverse();
  let sum = 0;
  splitter.forEach((element, index) => {
    sum += parseInt(element) * 256 ** index;
  });
  return sum;
}
/*-----------------------------------------------------------------
...
function gridTrip(arr, string) {
  ...
  const directions = { U: [0, 1], R: [1, 1], D: [0, -1], L: [1, -1] };
  let i = 0;
  while (i < string.length) {
    ...
    result[directions[dir][0]] += str * directions[dir][1];
  }
  return result;
}
/*-----------------------------------------------------------------
